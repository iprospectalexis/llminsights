/*
  # Update citation stats to filter by cited field (v2)
  
  1. Changes
    - Recreate audit_metrics_mv to exclude citations where cited=false
    - Only count citations where cited is true or null (missing field)
    - Skip initial refresh to avoid timeout
    
  2. Purpose
    - Accurately reflect SearchGPT's cited field in citation rate calculations
    - Treat null as true for backward compatibility with existing data
...
-- Recreate with cited field filter
