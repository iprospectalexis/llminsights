/*
  # Fix responses_sent calculation for OneSearch API

  1. Changes
    - Update audit_metrics_mv to count responses_sent correctly for both BrightData and OneSearch
    - BrightData uses snapshot_id field
    - OneSearch uses job_id field
    - Count responses where either snapshot_id OR job_id is not null

  2. Purpose
    - Fix the database icon visibility on Status page for OneSearch audits
    - Ensure accurate response tracking across both data providers

  3. Security
    - No security changes - maintains existing grants

  4. Notes
    - View is created empty (WHERE false) to avoid timeout
    - Will be populated by background refresh process
*/
